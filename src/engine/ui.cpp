...
namespace UI
{
    ...
    static void pushclip(float x, float y, float w, float h)
    {
        ...
        if (clipstack.length() >= 2)
        {
            c.intersect(clipstack[clipstack.length() - 2]);
        }
        c.scissor();
    }
...
#define BUILD(type, o, setup, contents)               \
    do                                                \
    {                                                 \
        if (buildparent)                              \
        {                                             \
...
#define loopchildrange(start, end, o, body) \
    do                                      \
    {                                       \
...
#define DOSTATE(flags, func)                                    \
    case flags:                                                 \
        func##children(cx, cy, mask, inside, setflags | flags); \
        return haschildstate(flags);
                DOSTATES
#undef DOSTATE
            }
            return false;
...
#undef DOSTATE
        ...
        void
        calcscissor(float x1, float y1, float x2, float y2, int &sx1, int &sy1, int &sx2, int &sy2, bool clip = true)
        {
            vec2 s1 = vec2(x1, y2).mul(sscale).add(soffset), s2 = vec2(x2, y1).mul(sscale).add(soffset);
...
#undef DOSTATE
    ...
    struct HorizontalList : Object
    {
        ...
        void adjustchildren()
        {
            ...
            float offset = 0, sx = 0, cspace = (w - subw) / max(children.length() - 1, 1),
    ...
    struct VerticalList : Object
    {
        ...
        void adjustchildren()
        {
            ...
            float offset = 0, sy = 0, rspace = (h - subh) / max(children.length() - 1, 1),
    ...
    struct Grid : Object
    {
        ...
        void adjustchildren()
        {
            ...
            int row = 0, column = 0;
            float offsety = 0, sy = 0, offsetx = 0, sx = 0, cspace = (w - subw) / max(widths.length() - 1, 1),
                  cstep = (w - subw) / widths.length(), rspace = (h - subh) / max(heights.length() - 1, 1),
...
#define BUILDCOLUMNS(type, o, setup, columndata, contents) \
    do                                                     \
    {                                                      \
        if (buildparent)                                   \
        {                                                  \
    ...
    struct Table : Object
    {
        ...
        void adjustchildren()
        {
            ...
            float offsety = 0, sy = 0, cspace = (w - subw) / max(widths.length() - 1, 1),
                  cstep = (w - subw) / widths.length(), rspace = (h - subh) / max(children.length() - 1, 1),
                  rstep = (h - subh) / children.length();
            loopchildren(o, {
                o->x = 0;
                o->y = offsety;
    ...
    struct Gradient : FillColor
    {
        ...
        void draw(float sx, float sy)
        {
            ...
            gle::begin(GL_TRIANGLE_STRIP);
            gle::attribf(sx + w, sy);
            (dir == HORIZONTAL ? color2 : color).attrib();
            gle::attribf(sx, sy);
            color.attrib();
            gle::attribf(sx + w, sy + h);
            color2.attrib();
            gle::attribf(sx, sy + h);
            (dir == HORIZONTAL ? color : color2).attrib();
            gle::end();
    ...
    static inline bool checkalphamask(Texture *tex, float x, float y)
    {
        ...
        int tx = clamp(int(x * tex->xs), 0, tex->xs - 1), ty = clamp(int(y * tex->ys), 0, tex->ys - 1);
    ...
    struct StretchedImage : Image
    {
        ...
        void draw(float sx, float sy)
        {
            ...
            float splitw = (minw ? min(minw, w) : w) / 2, splith = (minh ? min(minh, h) : h) / 2, vy = sy, ty = 0;
            loopi(3)
            {
                float vh = 0, th = 0;
                switch (i)
...
#undef DOSTATE
    ...
    static void setfval(ident *id, double val, uint *onchange = NULL)
    {
        ...
        {
            case ID_VAR:
                setvarchecked(id, int(clamp(val, double(INT_MIN), double(INT_MAX))));
                break;
            case ID_FVAR:
        ...
        if (onchange && (*onchange & CODE_OP_MASK) != CODE_EXIT)
        {
            execute(onchange);
        }
    }
    ...
    struct HorizontalSlider : Slider
    {
        ...
        void scrollto(float cx, float cy)
        {
            ...
            int step = int((val - vmin) / vstep), bstep = int(offset * (vmax - vmin) / vstep);
            if (step != bstep)
            {
        ...
        void adjustchildren()
        {
            ...
            int step = int((val - vmin) / vstep), bstep = int(button->x / (w - button->w) * (vmax - vmin) / vstep);
            if (step != bstep)
            {
    ...
    struct VerticalSlider : Slider
    {
        ...
        void adjustchildren()
        {
            ...
            int step = int((val - vmin) / vstep), bstep = int(button->y / (h - button->h) * (vmax - vmin) / vstep);
            if (step != bstep)
            {
        ...
        void previewslot(Slot &slot, VSlot &vslot, float x, float y)
        {
            ...
            if (slot.loaded)
            {
                ...
                if (vslot.detail)
                {
                    ...
                    if (!detail->slot->sts.empty())
                    {
                        detailtex = detail->slot->sts[0].t;
                    }
                }
            }
            else
...
#define DOSTATE(flags, func)                                                              \
    ...
    ICOMMANDNS("ui!" #func "+",                                                           \
               ...
               "ee",                                                                      \
               (uint * t, uint * f),                                                      \
               executeret(buildparent && buildparent->children.inrange(buildchild) &&     \
                                  buildparent->children[buildchild]->hasstate(flags)      \
                              ? t                                                         \
                              : f));                                                      \
    ICOMMANDNS("ui" #func "+",                                                            \
               uinext##func##_,                                                           \
               ...
               (uint * t, uint * f),                                                      \
               executeret(buildparent && buildparent->children.inrange(buildchild) &&     \
                                  buildparent->children[buildchild]->haschildstate(flags) \
                              ? t                                                         \
                              : f));                                                      \
    ICOMMANDNS("ui!" #func "+?",                                                          \
               uinextnot##func##__,                                                       \
               "tt",                                                                      \
               (tagval * t, tagval * f),                                                  \
               IFSTATEVAL(buildparent && buildparent->children.inrange(buildchild) &&     \
                              buildparent->children[buildchild]->hasstate(flags),         \
                          t,                                                              \
                          f));                                                            \
    ICOMMANDNS("ui" #func "+?",                                                           \
...
#undef DOSTATE
    ...
    ICOMMANDNS("uifocus+",
               ...
               (uint * t, uint *f),
               executeret(buildparent && buildparent->children.inrange(buildchild) &&
                                  TextEditor::focus == buildparent->children[buildchild]
                              ? t
                              : f));
    ICOMMANDNS("uifocus+?",
               uinextfocus__,
    ...
    ICOMMAND(uigroup, "e", (uint * children), BUILD(Object, o, o->setup(), children));

    ICOMMAND(uihlist, "fe", (float *space, uint *children), BUILD(HorizontalList, o, o->setup(*space), children));

    ICOMMAND(uivlist, "fe", (float *space, uint *children), BUILD(VerticalList, o, o->setup(*space), children));
    ...
    ICOMMAND(uigrid,
             "iffe",
             (int *columns, float *spacew, float *spaceh, uint *children),
             BUILD(Grid, o, o->setup(*columns, *spacew, *spaceh), children));

    ICOMMAND(uitableheader,
             "ee",
             (uint * columndata, uint *children),
    ...
    ICOMMAND(uitable,
             "ffe",
             (float *spacew, float *spaceh, uint *children),
    ...
    ICOMMAND(uioffset,
             "ffe",
             (float *offsetx, float *offsety, uint *children),
             BUILD(Offsetter, o, o->setup(*offsetx, *offsety), children));

    ICOMMAND(uifill,
             "ffe",
             (float *minw, float *minh, uint *children),
    ...
    ICOMMAND(uiclip,
             "ffe",
             (float *clipw, float *cliph, uint *children),
    ...
    ICOMMAND(uihscrollbar, "e", (uint * children), BUILD(HorizontalScrollBar, o, o->setup(), children));

    ICOMMAND(uivscrollbar, "e", (uint * children), BUILD(VerticalScrollBar, o, o->setup(), children));

    ICOMMAND(uiscrollarrow, "fe", (float *dir, uint *children), BUILD(ScrollArrow, o, o->setup(*dir), children));

    ICOMMAND(uiscrollbutton, "e", (uint * children), BUILD(ScrollButton, o, o->setup(), children));

    ICOMMAND(uihslider,
             "rfffee",
             (ident * var, float *vmin, float *vmax, float *vstep, uint *onchange, uint *children),
    ...
    ICOMMAND(uisliderarrow, "fe", (float *dir, uint *children), BUILD(SliderArrow, o, o->setup(*dir), children));

    ICOMMAND(uisliderbutton, "e", (uint * children), BUILD(SliderButton, o, o->setup(), children));

    ICOMMAND(uicolor,
             "iffe",
             (int *c, float *minw, float *minh, uint *children),
    ...
    ICOMMAND(uivgradient,
             "iiffe",
             (int *c, int *c2, float *minw, float *minh, uint *children),
             BUILD(Gradient,
                   o,
                   o->setup(Gradient::SOLID, Gradient::VERTICAL, Color(*c), Color(*c2), *minw, *minh),
                   children));

    ICOMMAND(uimodvgradient,
             "iiffe",
             (int *c, int *c2, float *minw, float *minh, uint *children),
             BUILD(Gradient,
                   o,
                   o->setup(Gradient::MODULATE, Gradient::VERTICAL, Color(*c), Color(*c2), *minw, *minh),
                   children));

    ICOMMAND(uihgradient,
             "iiffe",
             (int *c, int *c2, float *minw, float *minh, uint *children),
             BUILD(Gradient,
                   o,
                   o->setup(Gradient::SOLID, Gradient::HORIZONTAL, Color(*c), Color(*c2), *minw, *minh),
                   children));

    ICOMMAND(uimodhgradient,
             "iiffe",
             (int *c, int *c2, float *minw, float *minh, uint *children),
             BUILD(Gradient,
                   o,
                   o->setup(Gradient::MODULATE, Gradient::HORIZONTAL, Color(*c), Color(*c2), *minw, *minh),
                   children));
    ...
    ICOMMAND(uicolortext,
             "tife",
             (tagval * text, int *c, float *scale, uint *children),
             buildtext(*text, *scale, uitextscale, Color(*c), -1, children));

    ICOMMAND(uitext,
             "tfe",
             (tagval * text, float *scale, uint *children),
    ...
    ICOMMAND(uiwrapcolortext,
             "tfife",
             (tagval * text, float *wrap, int *c, float *scale, uint *children),
             buildtext(*text, *scale, uitextscale, Color(*c), *wrap, children));

    ICOMMAND(uiwraptext,
             "tffe",
             (tagval * text, float *wrap, float *scale, uint *children),
    ...
    ICOMMAND(uicontextfill,
             "ffe",
             (float *minw, float *minh, uint *children),
             BUILD(Filler, o, o->setup(*minw *FONTH *uicontextscale * 0.5f, *minh *FONTH *uicontextscale), children));

    ICOMMAND(uiwrapcolorcontext,
             "tfife",
             (tagval * text, float *wrap, int *c, float *scale, uint *children),
             buildtext(*text, *scale, FONTH *uicontextscale, Color(*c), *wrap, children));

    ICOMMAND(uiwrapcontext,
             "tffe",
             (tagval * text, float *wrap, float *scale, uint *children),
             buildtext(*text, *scale, FONTH *uicontextscale, Color(255, 255, 255), *wrap, children));

    ICOMMAND(uitexteditor,
             "siifsie",
             (char *name, int *length, int *height, float *scale, char *initval, int *mode, uint *children),
             BUILD(TextEditor,
                   o,
                   o->setup(name,
                            *length,
                            *height,
                            (*scale <= 0 ? 1 : *scale) * uitextscale,
                            initval,
                            *mode <= 0 ? EDITORFOREVER : *mode),
                   children));

    ICOMMAND(uifont, "se", (char *name, uint *children), BUILD(Font, o, o->setup(name), children));

    ICOMMAND(uiabovehud, "", (), {
        if (window)
        {
            window->abovehud = true;
        }
    });
    ...
    ICOMMAND(uifield,
             "riefe",
             (ident * var, int *length, uint *onchange, float *scale, uint *children),
    ...
    ICOMMAND(uiimage,
             "sffe",
             (char *texname, float *minw, float *minh, uint *children),
             BUILD(Image, o, o->setup(textureload(texname, 3, true, false), *minw, *minh), children));

    ICOMMAND(uistretchedimage,
             "sffe",
             (char *texname, float *minw, float *minh, uint *children),
                   ...
                   o,
                   {
                       ...
                       o->setup(tex,
                                ...
                                *minh,
                                ...
                                parsepixeloffset(croph, tex->ys));
                   },
                   children));

    ICOMMAND(uiborderedimage,
             "stfe",
             (char *texname, tagval *texborder, float *screenborder, uint *children),
             BUILD(BorderedImage,
                   o,
                   {
                       Texture *tex = textureload(texname, 3, true, false);
                       o->setup(tex, parsepixeloffset(texborder, tex->xs), *screenborder);
                   },
                   children));

    ICOMMAND(uitiledimage,
             "sffffe",
             (char *texname, float *tilew, float *tileh, float *minw, float *minh, uint *children),
             BUILD(TiledImage,
                   o,
                   {
    ...
    ICOMMAND(uimodelpreview,
             "ssffe",
             (char *model, char *animspec, float *minw, float *minh, uint *children),
             BUILD(ModelPreview, o, o->setup(model, animspec, *minw, *minh), children));

    ICOMMAND(uiplayerpreview,
             "iiiiffe",
             (int *model, int *color, int *team, int *weapon, float *minw, float *minh, uint *children),
             BUILD(PlayerPreview, o, o->setup(*model, *color, *team, *weapon, *minw, *minh), children));

    ICOMMAND(uiprefabpreview,
             "siffe",
             (char *prefab, int *color, float *minw, float *minh, uint *children),
             BUILD(PrefabPreview, o, o->setup(prefab, *color, *minw, *minh), children));

    ICOMMAND(uislotview,
             "iffe",
             (int *index, float *minw, float *minh, uint *children),
             BUILD(SlotViewer, o, o->setup(*index, *minw, *minh), children));

    ICOMMAND(uivslotview,
             "iffe",
             (int *index, float *minw, float *minh, uint *children),
    ...
    bool keypress(int code, bool isdown)
    {
        ...
        {
            case -1:
                action = isdown ? STATE_PRESS : STATE_RELEASE;
                hold = STATE_HOLD;
                break;
            case -2:
                action = isdown ? STATE_ALT_PRESS : STATE_ALT_RELEASE;
                hold = STATE_ALT_HOLD;
                break;
            case -3:
                action = isdown ? STATE_ESC_PRESS : STATE_ESC_RELEASE;
                hold = STATE_ESC_HOLD;
                break;
            case -4:
