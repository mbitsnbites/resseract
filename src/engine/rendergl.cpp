...
void gl_checkextensions()
{
    ...
    {
        hasPBO = true;
        if(glversion < 210 && dbgexts) logoutf("Using GL_ARB_pixel_buffer_object extension.");
    }
    ...
    if(glversion >= 300 || hasext("GL_ARB_vertex_array_object"))
    {
        ...
        hasVAO = true;
        if(glversion < 300 && dbgexts) logoutf("Using GL_ARB_vertex_array_object extension.");
    }
    else if(hasext("GL_APPLE_vertex_array_object"))
    {
        ...
        hasVAO = true;
        if(dbgexts) logoutf("Using GL_APPLE_vertex_array_object extension.");
    }
    ...
    if(glversion >= 300)
    {
        ...
        {
            hasEGPU4 = true;
            if(dbgexts) logoutf("Using GL_EXT_gpu_shader4 extension.");
        }
    ...
    else
    {
        ...
        {
            hasTF = true;
            if(dbgexts) logoutf("Using GL_ARB_texture_float extension.");
        }
        if(hasext("GL_ARB_texture_rg"))
        {
            hasTRG = true;
            if(dbgexts) logoutf("Using GL_ARB_texture_rg extension.");
        }
        if(hasext("GL_ARB_texture_compression_rgtc") || hasext("GL_EXT_texture_compression_rgtc"))
        {
            hasRGTC = true;
            if(dbgexts) logoutf("Using GL_ARB_texture_compression_rgtc extension.");
        }
        if(hasext("GL_EXT_packed_float"))
        {
            hasPF = true;
            if(dbgexts) logoutf("Using GL_EXT_packed_float extension.");
        }
        if(hasext("GL_EXT_gpu_shader4"))
        {
        ...
        if(hasext("GL_ARB_color_buffer_float"))
        {
            ...
            hasCBF = true;
            if(dbgexts) logoutf("Using GL_ARB_color_buffer_float extension.");
        }
        if(hasext("GL_EXT_draw_buffers2"))
        {
            ...
            hasDB2 = true;
            if(dbgexts) logoutf("Using GL_EXT_draw_buffers2 extension.");
        }
        if(hasext("GL_NV_conditional_render"))
        {
            ...
            hasCR = true;
            if(dbgexts) logoutf("Using GL_NV_conditional_render extension.");
        }
        if(hasext("GL_EXT_texture_integer"))
        {
            ...
            hasTI = true;
            if(dbgexts) logoutf("Using GL_EXT_texture_integer extension.");
        }
        if(hasext("GL_NV_half_float"))
        {
            hasHFV = hasHFP = true;
            if(dbgexts) logoutf("Using GL_NV_half_float extension.");
        }
        else
        {
            ...
            {
                hasHFV = true;
                if(dbgexts) logoutf("Using GL_ARB_half_float_vertex extension.");
            }
            if(hasext("GL_ARB_half_float_pixel"))
            {
                hasHFP = true;
                if(dbgexts) logoutf("Using GL_ARB_half_float_pixel extension.");
            }
        }
    }
    ...
    if(glversion >= 300 || hasext("GL_ARB_framebuffer_object"))
    {
        ...
        if(glversion < 300 && dbgexts) logoutf("Using GL_ARB_framebuffer_object extension.");
    }
    else if(hasext("GL_EXT_framebuffer_object"))
    {
        ...
        hasFBO = true;
        if(dbgexts) logoutf("Using GL_EXT_framebuffer_object extension.");

        if(hasext("GL_EXT_framebuffer_blit"))
        {
            glBlitFramebuffer_     = (PFNGLBLITFRAMEBUFFERPROC)        getprocaddress("glBlitFramebufferEXT");
            hasFBB = true;
            if(dbgexts) logoutf("Using GL_EXT_framebuffer_blit extension.");
        }
        if(hasext("GL_EXT_framebuffer_multisample"))
        {
            ...
            hasFBMS = true;
            if(dbgexts) logoutf("Using GL_EXT_framebuffer_multisample extension.");
        }
        if(hasext("GL_EXT_packed_depth_stencil") || hasext("GL_NV_packed_depth_stencil"))
        {
            hasDS = true;
            if(dbgexts) logoutf("Using GL_EXT_packed_depth_stencil extension.");
        }
    }
    ...
    if(glversion >= 300 || hasext("GL_ARB_map_buffer_range"))
    {
        ...
        hasMBR = true;
        if(glversion < 300 && dbgexts) logoutf("Using GL_ARB_map_buffer_range.");
    }
    ...
    if(glversion >= 310 || hasext("GL_ARB_uniform_buffer_object"))
    {
        ...
        useubo = 1;
        hasUBO = true;
        if(glversion < 310 && dbgexts) logoutf("Using GL_ARB_uniform_buffer_object extension.");
    }

    if(glversion >= 310 || hasext("GL_ARB_texture_rectangle"))
    {
        hasTR = true;
        if(glversion < 310 && dbgexts) logoutf("Using GL_ARB_texture_rectangle extension.");
    }
    ...
    if(glversion >= 310 || hasext("GL_ARB_copy_buffer"))
    {
        ...
        hasCB = true;
        if(glversion < 310 && dbgexts) logoutf("Using GL_ARB_copy_buffer extension.");
    }
    ...
    if(glversion >= 320 || hasext("GL_ARB_texture_multisample"))
    {
        ...
        hasTMS = true;
        if(glversion < 320 && dbgexts) logoutf("Using GL_ARB_texture_multisample extension.");
    }
    if(hasext("GL_EXT_framebuffer_multisample_blit_scaled"))
    {
        hasFBMSBS = true;
        if(dbgexts) logoutf("Using GL_EXT_framebuffer_multisample_blit_scaled extension.");
    }
    ...
    if(hasext("GL_EXT_timer_query"))
    {
        ...
        hasTQ = true;
        if(dbgexts) logoutf("Using GL_EXT_timer_query extension.");
    }
    else if(glversion >= 330 || hasext("GL_ARB_timer_query"))
    {
        ...
        hasTQ = true;
        if(glversion < 330 && dbgexts) logoutf("Using GL_ARB_timer_query extension.");
    }
...
#endif
        if(dbgexts) logoutf("Using GL_EXT_texture_compression_s3tc extension.");
    }
    ...
    {
        hasS3TC = true;
        if(dbgexts) logoutf("Using GL_EXT_texture_compression_dxt1 extension.");
    }
    if(hasext("GL_3DFX_texture_compression_FXT1"))
    {
        hasFXT1 = true;
        if(mesa) usetexcompress = max(usetexcompress, 1);
        if(dbgexts) logoutf("Using GL_3DFX_texture_compression_FXT1.");
    }
    if(hasext("GL_EXT_texture_compression_latc"))
    {
        hasLATC = true;
        if(dbgexts) logoutf("Using GL_EXT_texture_compression_latc extension.");
    }
    ...
    if(hasext("GL_EXT_texture_filter_anisotropic"))
    {
       ...
       hwmaxaniso = val;
       hasAF = true;
       if(dbgexts) logoutf("Using GL_EXT_texture_filter_anisotropic extension.");
    }
    ...
    if(hasext("GL_EXT_depth_bounds_test"))
    {
        ...
        hasDBT = true;
        if(dbgexts) logoutf("Using GL_EXT_depth_bounds_test extension.");
    }

    if(glversion >= 320 || hasext("GL_ARB_depth_clamp"))
    {
        hasDC = true;
        if(glversion < 320 && dbgexts) logoutf("Using GL_ARB_depth_clamp extension.");
    }
    else if(hasext("GL_NV_depth_clamp"))
    {
        hasDC = true;
        if(dbgexts) logoutf("Using GL_NV_depth_clamp extension.");
    }
    ...
    else
    {
        ...
        {
            hasTSW = true;
            if(dbgexts) logoutf("Using GL_ARB_texture_swizzle extension.");
        }
        if(hasext("GL_ARB_explicit_attrib_location"))
        {
            hasEAL = true;
            if(dbgexts) logoutf("Using GL_ARB_explicit_attrib_location extension.");
        }
        if(hasext("GL_ARB_occlusion_query2"))
        {
            hasOQ2 = true;
            if(dbgexts) logoutf("Using GL_ARB_occlusion_query2 extension.");
        }
    }
    ...
    if(glversion >= 330 || hasext("GL_ARB_blend_func_extended"))
    {
        ...
        hasBFE = true;
        if(glversion < 330 && dbgexts) logoutf("Using GL_ARB_blend_func_extended extension.");
    }
    ...
    else
    {
        ...
        {
            hasTG = true;
            if(dbgexts) logoutf("Using GL_ARB_texture_gather extension.");
        }
        if(hasext("GL_ARB_gpu_shader5"))
        {
            hasGPU5 = true;
            if(dbgexts) logoutf("Using GL_ARB_gpu_shader5 extension.");
        }
        if(hasext("GL_ARB_sample_shading"))
        {
            glMinSampleShading_ = (PFNGLMINSAMPLESHADINGPROC)getprocaddress("glMinSampleShadingARB");
            hasMSS = true;
            if(dbgexts) logoutf("Using GL_ARB_sample_shading extension.");
        }
        if(hasext("GL_ARB_draw_buffers_blend"))
        {
            ...
            hasDBB = true;
            if(dbgexts) logoutf("Using GL_ARB_draw_buffers_blend extension.");
        }
    }
    ...
    {
        hasES3 = true;
        if(glversion < 430 && dbgexts) logoutf("Using GL_ARB_ES3_compatibility extension.");
    }
    ...
    else
    {
        if(hasext("GL_ARB_debug_output"))
        {
            ...
            hasDBGO = true;
            if(dbgexts) logoutf("Using GL_ARB_debug_output extension.");
        }
    }
    ...
    if(glversion >= 430 || hasext("GL_ARB_copy_image"))
    {
        ...
        hasCI = true;
        if(glversion < 430 && dbgexts) logoutf("Using GL_ARB_copy_image extension.");
    }
    else if(hasext("GL_NV_copy_image"))
    {
        ...
        hasCI = true;
        if(dbgexts) logoutf("Using GL_NV_copy_image extension.");
    }
