...
struct ident
{
    ...
    ident(int t, const char *n, int flags) : type(t), valtype(VAL_NULL), flags(flags), name(n), code(NULL), stack(NULL)
    {
    }
...
#define _VARM(name, min, cur, max, scale, persist) \
    int name = cur * scale;                        \
    _VARF(name, _##name, min, cur, max, { name = _##name * scale; }, persist)
